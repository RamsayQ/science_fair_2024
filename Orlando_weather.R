...
## Analysis for Orlando, Florida during 2021-2024 ##############
################################################################
################################################################
...
# Download forcecasted weather forecast data through the Visual Crossing --------

# Define the base URL
base_url <- "https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/32836/"
...
for (i in 0:(as.numeric(end_date - start_date))) {
  ...
  target_end_date <- target_start_date + days(15)
...
# Define the API URL
api_url <- "https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/32836/2021-01-01/2024-03-12?unitGroup=us&include=days&key=7ZNU9ZCTWVVPRND4YBZAA3QNV&contentType=csv"
...
orlando_plot_max <- ggplot(summary_stats_max, aes(x = factor(`Forecast Horizon (days)`), y = `Reality Minus Forecast (average)`)) +
  ...
  ggtitle("Orlando, FL: High Temperature Forecast Accuracy") +
...
ggsave("orlando_plot.max.png", orlando_plot_max, dpi = 500)

# Create a bar plot for average values - min temps
orlando_plot_min <- ggplot(summary_stats_min, aes(x = factor(`Forecast Horizon (days)`), y = `Reality Minus Forecast (average)`)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  geom_text(aes(label = round(`Reality Minus Forecast (average)`, 2)), 
            position = position_stack(vjust = 0.5), 
            color = "red", size = 3) +  # Add text with red color
  labs(x = "Forecast Horizon (days)", y = "Reality Minus Forecast (average)") +
  ggtitle("Orlando, FL: Low Temperature Forecast Accuracy") +
...
ggsave("orlando_plot.min.png", orlando_plot_min, dpi = 500)

### Forecast errors by season

# Max temperatures
orlando_plot_seaons_max <- ggplot(seasonal_averages_max, aes(x = season, y = average_value)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  geom_text(aes(label = round(average_value, 2)), 
            position = position_stack(vjust = 0.5), 
            color = "red", size = 3) +
  labs(x = "", y = "Reality Minus Forecast (average)") +
  ggtitle("Orlando, FL: Forecast Accuracy by Seasons") +
...
ggsave("orlando_plot_seasons.max.png", orlando_plot_max, dpi = 500)

# Min temperatures
orlando_plot_seasons_min <- ggplot(seasonal_averages_min, aes(x = season, y = average_value)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  geom_text(aes(label = round(average_value, 2)), 
            position = position_stack(vjust = 0.5), 
            color = "red", size = 3) +
...
ggsave("orlando_plot_seasons_min.png", orlando_plot_seasons_min, dpi = 500)
